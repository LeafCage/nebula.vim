...
function! s:init_args(variadic) "{{{
  ...
  if get(l:, 'pluginroot', '')==''
    let inference = nebula_l#lim#misc#infer_plugin_pathinfo(expand('%'))
    if inference=={}
      return {}
    end
  ...
  if get(l:, 'pluginname', '')==''
    if !exists('inference')
      let inference = nebula_l#lim#misc#infer_plugin_pathinfo(pluginroot)
      if inference=={}
        return {}
      end
    end
...
"========================================================
"Main:
function! nebula_l#lim#alzplugin#analyze(...) "{{{
